...
class BrkFovea_1:
    ...
    def handleuser(self, switch):
        ...
        if self.process_chainge < 1:  # Check------------------ 2 to 1
            ...
            if not self.skip_event:
                ...
                time.sleep(0.2)
                # globaladc.buzzer_3()
                if not self.rptloop:  # ----False, true inter change
    ...
    def Load(self):
        self.patentActionflabel = tk.Label(self.frame, text='Increment Null Setting until\nPatient Reports no flicker,\nPress resume when done', font=Font1, bg='white')
        ...
        self.saveButton = tk.Button(self.content_frame, text="Save", bg='#a0f291', command=self.handleSave, font=Font, width=10)
        self.saveButton.place_forget()
    ...
    def show(self):
        ...
        # globaladc.brk_fovea_1_screen_initialize()  # run this while loading brk fovea 1st screen
        self.skip_event = True
        self.threadCreated = False
        self.run_therad()
        self.process = 0
